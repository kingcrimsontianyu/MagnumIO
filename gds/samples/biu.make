...
ARCH ?= $(shell uname -m)
CUDA_PATH   := /home/coder/.conda/envs/rapids/targets/sbsa-linux
CUDA_PATH_2 := /home/coder/.conda/envs/rapids
INCLUDES = -I $(CUDA_PATH)/include/ -I $(CUDA_PATH_2)/include/
ifeq ($(ARCH),aarch64)
	CUFILE_PATH ?= $(CUDA_PATH)/lib/
	CUFILE_INCLUDE_PATH ?= $(CUDA_PATH)/include/
else
...
LDFLAGS_STATIC     :=  $(CUFILE_LIB_STATIC) -L $(CUDA_PATH)/lib/stubs -lcuda $(CUDART_STATIC) -Bdynamic -lrt -ldl
INSTALL_GDSSAMPLES_PREFIX = /usr/local/gds/samples
NVCC          :=$(CUDA_PATH_2)/bin/nvcc
